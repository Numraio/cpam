...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Only allow retrying FAILED calculations
    if (calculation.status !== 'FAILED') {
      return res.status(400).json({ error: `Cannot retry calculation with status ${calculation.status}` });
    }

    // Create a new calculation batch with the same configuration
    const newCalculation = await prisma.calcBatch.create({
      data: {
        tenantId: team.id,
        pamId: calculation.pamId,
    ...
    return res.status(200).json({ calculation: newCalculation });
  } catch (error: any) {
